...
public class FormatingUtilities {
	...
	public static String formatObstacleGroups(int arrayIndex, String arrayOfValues, String obstacleType, String oldGroups) {
		...
		if (!arrayOfValues.isEmpty()) {
			...
			// Test to see if there are more than one new group (by checking for the ",:" separator). 
			//If there is only one new group than just return the one new group.
			if (arrayOfValues.contains(",:")) {
				...
				for (int j = 0; j < newGroups.size(); j++) {
					finalHtml = finalHtml + "<input  type='checkbox' name='" + obstacleType + "obsgrps[" + arrayIndex + "]' value=':" + newGroups.get(j).replaceAll("'", "&#39;") + "' checked><label>" + newGroups.get(j)
							+ "</label> <br> \n";
				}
			} else {
				//Create HTML for the one new groups. 
				finalHtml = "<input  type='checkbox' name='" + obstacleType + "obsgrps[" + arrayIndex + "]' value=':" + arrayOfValues.replaceAll("'", "&#39;") + "' checked><label>" + arrayOfValues.replaceAll(",", ", ")
						+ "</label> <br> \n";
			}
		}
        ...
        if (arrayOfGroups.isEmpty()) {
         //If there are no groups at all then set to the empty string.
			...
			newGroups = "";
	         ...
	         //If there are only new groups set the groups to the new group variable.

			oldGroups = "";
			newGroups = arrayOfGroups.substring(1);
			
		} else if (!arrayOfGroups.contains(":")){
	         //If there are no new groups then set the groups to the old group variable.

			oldGroups = arrayOfGroups;
			newGroups = "";			
			
		} else {
	         //If there are new groups and old groups then split them and add them to the appropriate variable.
