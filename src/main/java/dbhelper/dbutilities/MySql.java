	...
	public static Connection connect() {
		...
		try {
			...
			if (InetAddress.getLocalHost().getHostName().matches(".*jpcloudusa008.*")) {
				...
				conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/ukram123_unhcr_legaltool", "ukram123_1", "%%bac0n2013");
			} else if (InetAddress.getLocalHost().getHostName().matches(".*xjdz3.*")) {
				// Connection for Daily Razor host.
				conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/ukramcom_unhcr_legaltool", "ukramcom_1", "%%bac0n2013");
			} else {

				// Connection for my local machine or Arvixe.
