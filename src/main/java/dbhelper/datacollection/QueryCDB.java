...
public class QueryCDB {
	...
	public static Country getNarrativeObstacles(Country countryObj) {
		Connection c = MySql.connect();
		String countryName = countryObj.getCountryName();
		...
		try {
			...
			String sql = "SELECT * FROM obstacles_c1_narrative WHERE CountryName = ?";
			...
			if (rs.isBeforeFirst()) {
				...
				countryObj.setNarrativeObstacles(allFields);
				...
				countryObj.setNarrativeObstacles(noFields);
			}
			rs.close();
			pst.close();
